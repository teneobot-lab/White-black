    ...
    <div className="space-y-6 pb-10 max-w-[1200px] mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-soft border border-card-border dark:border-slate-800">
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-xl text-white shadow-sm ${activeTab === 'Outbound' ? 'bg-primary' : 'bg-secondary'}`}><ShoppingCart size={20} /></div>
          <div>
      ...
      {message && (
        <div className={`fixed bottom-10 right-10 p-5 rounded-2xl shadow-soft-lg flex items-center gap-3 animate-in slide-in-from-bottom-5 duration-300 border border-white/20 backdrop-blur-md ${message.type === 'success' ? 'bg-emerald-500/90 text-white' : 'bg-red-500/90 text-white'}`}>
