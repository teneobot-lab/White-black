...
const Transactions: React.FC = () => {
  ...
  // State Input Item - Diinisialisasi dengan nilai pasti untuk menghindari undefined
  ...
  // Header Info - Ramping & Terstruktur
  ...
  const addToCart = () => {
    ...
    // Reset selection ke state default yang bersih
    setQuantity(0); 
  ...
  return (
    <div className="space-y-6 pb-10 max-w-[1200px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-soft border border-card-border dark:border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-muted-gray uppercase tracking-widest mt-1">Movement Type: {activeTab}</p>
          </div>
        </div>
        ...
        <div className="p-1 bg-surface dark:bg-slate-800 rounded-xl flex gap-1">
          <button 
            ...
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === 'Outbound' ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-muted-gray hover:text-navy'}`}
          >
            PENGELUARAN
          </button>
          <button 
            onClick={() => setActiveTab('Inbound')}
            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === 'Inbound' ? 'bg-white dark:bg-slate-700 text-secondary shadow-sm' : 'text-muted-gray hover:text-navy'}`}
          >
            PENERIMAAN
          </button>
        </div>
      ...
      {/* Accurate-Style Header (Voucher Info) */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="space-y-1.5">
            ...
            <input 
              ...
              className="w-full px-3 py-2 bg-surface dark:bg-slate-950 border border-card-border dark:border-slate-800 rounded-lg text-xs outline-none focus:ring-1 focus:ring-primary/30"
            />
          </div>
          <div className="space-y-1.5">
            <label className="flex items-center gap-2 text-[10px] font-black text-muted-gray uppercase tracking-wider">
              <Calendar size={12} className="text-primary" /> Tanggal Transaksi
            </label>
            <input 
              type="date" 
              value={details.date} 
          ...
          <div className="space-y-1.5">
            <label className="flex items-center gap-2 text-[10px] font-black text-muted-gray uppercase tracking-wider">
              <Hash size={12} className="text-primary" /> Nomor Referensi (SJ/PO)
            </label>
            <input 
              type="text" 
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-soft border border-card-border dark:border-slate-800 overflow-hidden">
        ...
        <div className="p-3 bg-surface/50 dark:bg-slate-950/50 border-b border-card-border dark:border-slate-800 grid grid-cols-12 gap-3 items-end">
          <div className="col-span-12 md:col-span-6 relative">
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Cari Produk</label>
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full pl-9 pr-3 py-2 bg-white dark:bg-slate-900 border border-card-border dark:border-slate-800 rounded-lg text-xs outline-none focus:ring-1 focus:ring-primary/30 transition-all"
              />
            </div>
            {isDropdownOpen && searchTerm && filteredItems.length > 0 && (
              <div className="absolute top-full left-0 right-0 z-50 bg-white dark:bg-slate-900 mt-1 rounded-lg shadow-soft-lg border border-card-border dark:border-slate-800 overflow-hidden max-h-48 overflow-y-auto">
                {filteredItems.map(item => (
                  <button key={item.id} onClick={() => {setSelectedItem(item); setSearchTerm(item.name); setIsDropdownOpen(false);}} className="w-full text-left px-4 py-2 hover:bg-surface dark:hover:bg-slate-800 text-xs font-medium transition-colors border-b last:border-0 border-slate-50 dark:border-slate-800">
          ...
          <div className="col-span-4 md:col-span-2">
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Kuantitas</label>
            <input 
              type="number" 
              value={quantity}
              onChange={(e) => setQuantity(Number(e.target.value))}
              className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-card-border dark:border-slate-800 rounded-lg text-xs outline-none focus:ring-1 focus:ring-primary/30"
            />
          </div>
          <div className="col-span-4 md:col-span-2">
            ...
            <div className="relative">
              <select 
                ...
                className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-card-border dark:border-slate-800 rounded-lg text-xs outline-none focus:ring-1 focus:ring-primary/30 appearance-none cursor-pointer"
              >
          ...
          <div className="col-span-4 md:col-span-2">
            <button 
              ...
              className="w-full py-2 bg-primary text-white rounded-lg text-xs font-bold shadow-sm hover:bg-blue-600 disabled:opacity-30 transition-all active:scale-95 flex items-center justify-center gap-2"
            >
              <Plus size={14} /> TAMBAH
            </button>
          </div>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center opacity-30">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest text-muted-gray">Belum ada item ditambahkan</p>
                    </div>
                  </td>
                </tr>
              )}
        ...
        <div className="p-6 bg-surface/30 dark:bg-slate-950/30 border-t border-card-border dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex gap-8">
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Item</span>
              <span className="text-sm font-black text-navy dark:text-white">{cart.length} SKU</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Kuantitas</span>
          ...
          <button 
            ...
            className={`px-10 py-3 rounded-xl font-black text-[11px] text-white shadow-soft-lg hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-2 uppercase tracking-widest ${activeTab === 'Inbound' ? 'bg-secondary' : 'bg-primary'} disabled:opacity-30`}
          >
      ...
      {/* Alert Messages */}
      {message && (
        <div className={`fixed bottom-10 right-10 p-5 rounded-2xl shadow-soft-lg flex items-center gap-3 animate-in slide-in-from-bottom-5 duration-300 border ${
